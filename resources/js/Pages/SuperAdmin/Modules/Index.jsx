...
export default function Index({ modules }) {
  return (
    <AppLayout title="Módulos">
      <Head title="Módulos" />
      <Card>
        <CardHeader title="Catálogo de módulos" description="Estes módulos podem ser liberados individualmente nas licenças contratuais." />
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {modules.map((module) => (
            <div key={module.id} className="rounded-3xl border border-slate-200 bg-slate-50 p-5">
              <div className="mb-3 flex items-start justify-between gap-3"><h3 className="font-bold text-slate-950">{module.name}</h3><Badge tone={module.active ? 'green' : 'gray'}>{module.active ? 'ativo' : 'inativo'}</Badge></div>
